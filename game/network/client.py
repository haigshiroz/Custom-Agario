    ...
    def connect_to_game(self, get_attrs):
        ...
        try:
            ...
            while True:
                ...
                should_shoot = action == 5
                direction_to_go = action if action < 5 else None


                mouse_pos = (0, 0)  # Assuming dummy input
                mouse_pos, keys = get_direction_and_keys(mouse_pos, direction=direction_to_go, shoot=should_shoot)
                print(f"[AI] State: {prev_state}, Action: {action}, Shoot: {should_shoot}, Direction: {direction_to_go}")
